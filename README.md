...
| Intake | `/api/intake` | multipart upload → deterministic CSV validation → suggestions (tax memory + rules + advisory AI) → decide → gate-checked commit → supersede; adjustment approve/reject learning signals; XLSX via `/xlsx-upload` → `/preview` → `/column-map` (behind `INTAKE_XLSX`, remembered in `intake_column_maps`); sign-convention gate via `/batches/:id/sign-convention` + `/confirm` + `/reject` (behind `INTAKE_SIGN_CONVENTION`, reviewer-decided × −1 correction, audited); prior-period bridge via `/batches/:id/prior-bridge` + `/bridge/items/:itemId/resolve` (behind `INTAKE_PRIOR_BRIDGE`, rename/new → proposals, mismatch blocks commit); review surface on the Intake page (`/intake`: Excel upload → map, sign totals table + confirm/reject, bridge diff + resolve — each gated on its flag); asset register ingest via `/entities/:entityId/assets` (behind `INTAKE_ASSET_REGISTER`, CAA 2001 pools fed to the workbench run, `missing_asset_register` when absent) |
