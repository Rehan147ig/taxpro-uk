...
**Status:** Verification complete — build verified, benchmark harnesses green, integration E2E green, deployment hardening verified, UK Phases C + D (tax-close workbench + filing-ready handoff) shipped and verified against live Postgres/Redis; UK Phase 1 (enterprise data intake) + Tax Intelligence Layer shipped 2026-08-05; Phase 1E/1G follow-up (run↔batch linkage, journal workpapers export, agent framework adoption, operator UI) shipped 2026-08-05. NOT filing-ready.
**Date:** 2026-08-01 (re-verified 2026-08-02 and 2026-08-03; UK-first product reset 2026-08-04; UK Phase C shipped 2026-08-04; UK Phase D shipped 2026-08-04; enterprise intake + tax intelligence layer shipped 2026-08-05; run↔batch linkage + journal export + operator UI shipped 2026-08-05)
**Branch:** master
**Test Suite:** 615 tests passing (118 tax-engine + 408 API + 89 tax-engine-enterprise), 0 failures
...
| Unit tests | `npm test` | 615/615 PASS (118 engine + 408 API + 89 tax-engine-enterprise) |
...
| Phase 1E/1G API | `npm test -w @taxpro/api` | 408/408 PASS (33 files) — `journals-export` added (10 pure CSV/dialect/escaping tests + 7 live-DB route tests incl. cross-tenant 404 and balanced-line control); run↔batch linkage exercised across `phase-c-workbench`, `phase-d-handoff`, `phase-e-intake`, `phase-f-intelligence`, `phase7-security`, `audit`, `integration-rls`, `ai-subagents` (all green with `import_batch_id` in the schema); migration 0021 applied on live Postgres |
...
**Re-verified 2026-08-05 after Phase 1E/1G (run↔batch linkage + journal workpapers export + agent framework adoption + operator UI):** API 408/408 (33 files — new `journals-export` suite: 10 pure tests over `buildJournalExport`/`journalsToCsv` (Xero/QBO/NetSuite/generic dialects, escaping) + 7 live-DB route tests (`GET /api/export/journals/:resultId?format=`, balanced-line controls, cross-tenant 404, role guard)). Shipped: migration `0021_provision_run_batch_linkage` (`provision_runs.import_batch_id` FK `import_batches` ON DELETE SET NULL, applied live — note the migrator resolves `<tag>.sql`, so the file is `provision_run_batch_linkage.sql`); `lib/import-batch-link.ts` (auto-link committed non-superseded batches via accounting-period containment, `PATCH /api/workbench/runs/:id/import-batch` with 409-on-locked guard, recalc inheritance, provision `/run` auto-link); `modules/export` journal workpapers (engine `journalEntries` + derived FRS 102 S29 fallback, JSON + 4 CSV dialects); `agent/subagents/framework-adapters.ts` adopting the eve framework (`runAgent` + `runReadOnly` + discovery events) for mapping/audit-defense/credit-miner swarm callbacks; operator UI (Intake page with upload→validate→suggest→decide→commit + adjustment review dashboard, Provenance visualizer with journal JSON/CSV download, provenance links from workbench results, nav entry). Monorepo lint 5/5, `tsc` clean (api + web), web production build clean, `eval:uk` 9/9 (mean ETR delta 1.3 bp, deferred 0 bp). **Still NOT filing-ready and not production-ready:** no CT600/iXBRL XSD validation, no CTO GovTalk XML submission, no MTD; external CPA review and a formal security audit remain required; paid-pilot and production gates (Phase E + Phase 11 §gates) remain open.
